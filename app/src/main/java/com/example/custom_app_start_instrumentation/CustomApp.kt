...
// Holder object for test data generation
object AppStartTransactionHolder {
    /**
     * Simulates an app start transaction with realistic timing data.
     * This is for TESTING only - to generate sample data in Sentry.
     * The real app start is tracked automatically by AppStartTracker.
     */
    fun simulateAppStart(
        durationMs: Long = (800..2000).random().toLong(),
        isColdStart: Boolean = listOf(true, false).random()
    ) {
        val startType = if (isColdStart) "cold" else "warm"

        // Create transaction with null timeouts to reduce timer threads
        val transactionOptions = TransactionOptions().apply {
            deadlineTimeout = null
            idleTimeout = null
        }

        // Use "ui.load" operation for Mobile Vitals recognition
        val transaction = Sentry.startTransaction(
            "MainActivity.simulated",
            "ui.load",
            transactionOptions
        )

        // Add realistic app start phases following Sentry's structure
        val phases = listOf(
            "app.start.process.init" to (50..150).random().toLong(),
            "app.start.application.create" to (100..300).random().toLong(),
            "app.start.activity.create" to (80..250).random().toLong(),
            "app.start.first.frame" to (200..600).random().toLong()
        )

        phases.forEach { (operation, duration) ->
            Thread.sleep(duration)
            val span = transaction.startChild(operation)
            span.description = operation.split(".").joinToString(" ") {
        ...
        // Add tags and data following Sentry's spec and Mobile Vitals requirements
        transaction.setTag("start_type", startType)
        transaction.setTag("simulated", "true")
        transaction.setTag("ui.load.type", startType)
        transaction.setData("app_start_type", startType)
        transaction.setData("duration_ms", durationMs)

        // Set measurements that Mobile Vitals recognizes
        val measurementName = if (isColdStart) "app_start_cold" else "app_start_warm"
        transaction.setMeasurement(measurementName, durationMs.toDouble(), MeasurementUnit.Duration.MILLISECOND)
        transaction.setMeasurement("time_to_initial_display", durationMs.toDouble(), MeasurementUnit.Duration.MILLISECOND)
        transaction.setMeasurement("time_to_full_display", durationMs.toDouble(), MeasurementUnit.Duration.MILLISECOND)
